...
model.MC.wavelength               = 500; % [nm] Excitation wavelength, used for determination of optical properties for excitation light
...
for iRun = 1:n
  ...
  Rt_mismatched_(iRun) = Rt_mismatched;
...
fprintf('Rt mismatched = %.6f +- %.6f (n = %d, total time elapsed = %d s)\n\n',mean(Rt_mismatched_),std(Rt_mismatched_)/sqrt(n),n,round(t_Mismatched));

%% 
% Compute mua from formula. The parameters come from graph, and depend on
% the wavelength of excitation light.
chb1 = 0.01; % Oxyhemoglobin concentration (Placeholder value for now)
chb2 = 0.01; % Deoxyhemoglobin concentration (Placeholder value for now)
cm   = 1; % Melanin concentration (Placeholder value for now)
ehb1 = 10; % Oxyhemoglobin absorption coefficient (/mm) at the test wavelength
ehb2 = 10; % Deoxyhemoglobin absorption coefficient (/mm) at the test wavelength
km   = 10;  % Exponential decay factor
lambda_o = 400; % Constant
global mua;
mua = chb1*ehb1 + chb2*ehb2 + cm*exp(-1*km*(model.MC.wavelength - lambda_o)/lambda_o);
...
% X is of shape (model.G.nx, model.G.ny, model.G.nz)
...
function mediaProperties = mediaPropertiesFunc(wavelength,parameters)
  ...
  mediaProperties(j).mua   = 10*mua; % [cm^-1]
  mediaProperties(j).mus   = 12.7; % [cm^-1]
  mediaProperties(j).g     = 0.70;
  mediaProperties(j).n     = 1.42;
end